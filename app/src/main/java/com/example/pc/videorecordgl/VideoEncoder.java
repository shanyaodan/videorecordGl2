...
import static com.example.pc.videorecordgl.OutputConfig.TIMEOUT_USEC;
import static com.example.pc.videorecordgl.XMediaMuxer.TRACK_VIDEO;
...
class VideoEncoder extends Thread {
    ...
    Surface getInputSurface() {
        LogUtils.i("VideoEncoder".getClass(), "mInputSurface isEmpty? " + (mInputSurface == null));
            ...
            synchronized (this) {
                ...
                switch (encoderStatus) {
                    case MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED:
                        LogUtils.i("VideoEncoder".getClass(), "INFO_OUTPUT_BUFFERS_CHANGED");
                        break;
                    case MediaCodec.INFO_OUTPUT_FORMAT_CHANGED:
                        MediaFormat newFormat = mVideoEncoder.getOutputFormat();
                        mMediaMuxer.addMediaTrack(TRACK_VIDEO, newFormat);
                         LogUtils.i("VideoEncoder".getClass(), "New format " + newFormat);
                        break;
                    case MediaCodec.INFO_TRY_AGAIN_LATER:
                        try {
                            Thread.sleep(10);       // wait 10ms
                             LogUtils.i("VideoEncoder".getClass(), "dequeueOutputBuffer timed out!");
                    ...
                    default:
                        final ByteBuffer outputBuffer = mVideoEncoder.getOutputBuffers()[encoderStatus];
                         LogUtils.i("VideoEncoder".getClass(), "We can't use this buffer but render it due to the API limit, " + outputBuffer);
                        if ((mVideoBufferInfo.flags & MediaCodec.BUFFER_FLAG_CODEC_CONFIG) != 0) {
                            mVideoBufferInfo.size = 0;
                        }
                        if (mVideoBufferInfo.size != 0 && mMediaMuxer != null) {
                             LogUtils.i("VideoEncoder".getClass(), "| timestamp:: " + mVideoBufferInfo.presentationTimeUs / 1000 + "ms |");

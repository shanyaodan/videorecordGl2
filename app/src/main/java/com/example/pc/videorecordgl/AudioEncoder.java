...
import static com.example.pc.videorecordgl.XMediaMuxer.TRACK_AUDIO;
    ...
    private void encode(final ByteBuffer buffer, final int length, final long presentationTimeUs) {
        ...
        switch (encoderStatus) {
            case MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED:
                LogUtils.i(getClass(), "INFO_OUTPUT_BUFFERS_CHANGED");
                break;
            case MediaCodec.INFO_OUTPUT_FORMAT_CHANGED:
                MediaFormat newFormat = mAudioEncoder.getOutputFormat();
                mMediaMuxer.addMediaTrack(TRACK_AUDIO, newFormat);
               LogUtils.i(getClass(), "New format " + newFormat);
                break;
            case MediaCodec.INFO_TRY_AGAIN_LATER:
                try {
                    Thread.sleep(10);       // wait 10ms
                   LogUtils.i(getClass(), "dequeueOutputBuffer timed out!");
            ...
            default:
               ...
               LogUtils.i(getClass(), "We can't use this buffer but render it due to the API limit, " + outputBuffer);
                if ((mAudioBufferInfo.flags & MediaCodec.BUFFER_FLAG_CODEC_CONFIG) != 0) {
                    mAudioBufferInfo.size = 0;
                }
                if (mAudioBufferInfo.size != 0 && mMediaMuxer != null) {
                   LogUtils.i(getClass(), "timestamp:: " + mAudioBufferInfo.presentationTimeUs / 1000 + "ms");
